...
function init_pagos_gateway() {
	...
	class woocommerce_pagos extends WC_Payment_Gateway {
		public function __construct() {
			...
			$this->firmas			= $this->settings['firma'];
		...
		function init_form_fields() {
			$this->form_fields = array(
				'enabled' => array(
					...
					'type' => 'checkbox',
					'label' => __( 'Enable PagosOnline', 'woothemes' ),
					'default' => 'yes'
					),
				'title' => array(
					...
					'default' => __( 'PagosOnline', 'woothemes' )
					),
				'Description'=>array(
				...
				'usuarioId'=>array(
					'title' => __( 'PagosOnline user account number', 'woothemes' ),
					'type' => 'text',
		...
		*/
		public function admin_options() {
			...
			?>
			<h3><?php _e('PagosOnline', 'woothemes'); ?></h3>

			<?php _e('<p>PagosOnline works by sending the user to <a href="http://www.pagosonline.com">pagosonline</a> to enter their payment information. Instructions on how to set up the pagos account settings can be found in the readme.txt.</p>', 'woothemes'); ?>
			...
			<div class="updated inline">
				...
				<p><?php _e('Also note, if you offer free shipping for tangible (i.e. physical) products you need to add a New Shipping Method in your PagosOnline account (see the shipping section in your pagos account). Set the pricing to Free and then choose which countries this method should apply to.', 'woothemes'); ?></p>
			</div>
		...
		**/
		public function generate_pagos_form( $order_id ) {
			...
			jQuery(function(){
				jQuery("body").block(
				{
					message: "<img src=\"'.$woocommerce->plugin_url().'/assets/images/ajax-loader.gif\" alt=\"Redirecting...\" />'.__('Thank you for your order. We are now redirecting you to pagosOnline to make payment.', 'woothemes').'",
					overlayCSS:
					{
						background: "#fff",
						opacity: 1
